...
export function useCategoryMovers() {
  return useQuery({
    queryKey: [...ANALYTICS_QUERY_KEY, "category-movers"],
    queryFn: getCategoryMovers,
  });
}

export function useSpendPace() {
  return useQuery({
    queryKey: [...ANALYTICS_QUERY_KEY, "spend-pace"],
    queryFn: getSpendPace,
  });
}
