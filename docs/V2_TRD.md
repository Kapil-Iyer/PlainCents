...
### 12.5 Cold Start
Overall eligibility: `months_available >= 3` (`TransactionRepository.count_distinct_months`), matching V1's `aggregate_monthly` raise threshold (`forecast.py`) exactly, per constraint #8/PRD §21. **ML-F AMENDMENT** (reports/ml/ML_F_SELECTION_RECORD.json's forecasting_selection): this threshold was 12 through ML-D; lowered to 6 for ML-F, and `backend/services/forecast_service.py`'s `MONTHS_REQUIRED` and `pipeline/forecast.py`'s `aggregate_monthly()` threshold were changed together. **ML-G AMENDMENT:** lowered again to 3 — the mathematical minimum of the selected 3-month rolling mean (one full window), not an accuracy-equivalence result. Both enforcement points moved together again, and `tests/backend/services/test_forecast_three_month_minimum.py` now asserts the two constants are equal, so they cannot silently drift — the latter is called directly by `ForecastService.run_forecast()`, so the two were never independently adjustable without one becoming a dead/unreachable check. Cold start surfaces as HTTP 200 with `{status: "cold_start", months_available, months_required: 3}` from `GET /api/forecasts/status` — never an exception. Per-category sparse history: the ML-F-selected 3-month rolling mean (`ml/forecasting/baselines.py::rolling_mean_predict`) has no rolling-window feature `dropna` requirement of its own (unlike the retired RF path) — `run_forecast()` maps a category to `is_available: false, unavailable_reason: "insufficient_history"` only when it has zero recorded occurrences at all, never merely sparse, matching ML-C's Naive-era behavior exactly.
...
| Forecast requires ≥3 months (was 12, then 6) | §12.5 — surfaced as cold-start; 3 is the selected rolling mean's window, i.e. the mathematical minimum |
