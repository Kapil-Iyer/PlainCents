...
- Forecasting requires a minimum amount of historical data (V1 enforced at least 12 unique months); V2 must treat falling short of it as a defined cold-start state, not an exception. **ML-F AMENDMENT** (reports/ml/ML_F_SELECTION_RECORD.json's forecasting_selection, per this section's own "unless later technical validation demonstrates it must change" clause): lowered to **6 unique months**. **ML-G AMENDMENT:** lowered again to **3 completed months**. Three is the MATHEMATICAL MINIMUM of the selected 3-month rolling mean — exactly one full window — and is adopted for that reason alone. It is explicitly NOT a finding that three months forecasts as accurately as six, nine or twelve: the history-length sensitivity experiments truncated to 6/9/12/18 months and never evaluated a three-month history at all. The previous 6-month figure was itself a usability judgement rather than an evidence-based floor, which is why it was reconsidered. The ML-F-selected 3-month rolling mean mathematically needs only 3 months and was found pooled-WAPE-stable across truncated history from 6 to 18 months (identical to how the prior Naive lag-1 selection behaved under the same test) — 6 months is a product-history usability floor, not an accuracy claim; a smaller number was not chosen because a forecast built from a single month or two is too thin a product experience to present as reliable, not because the model requires more.
